module IntenseDebateComments
  module ViewHelpers
    ...
    def id_comments(obj, options = {})
      ...
      concat_line raw('<script type="text/javascript">')
      concat_line raw("  var idcomments_acct = '#{ options[:acct_no] }';")
      concat_line raw("  var idcomments_post_id = '#{ obj.id }';")
      concat_line raw("  var idcomments_post_url #{ "='%s'" % options[:url] unless options[:url].nil? };")
      concat_line raw("  var idcomments_post_title #{ "='%s'" % options[:title] unless options[:title].nil? };")
      concat_line raw('</script>')
      concat_line raw("<span id=\"IDCommentsPostTitle\" style=\"display:none\"></span>\n")
      concat_line raw("<script type=\"text/javascript\" src=\"http://www.intensedebate.com/js/genericCommentWrapperV2.js\"></script>\n")
      nil
    end
    ...
    def id_comments_link(obj, options = {})
      ...
      concat_line raw("  var idcomments_acct = '#{ options[:acct_no] }';")
      concat_line raw("  var idcomments_post_id = '#{ obj.id }';")
      concat_line raw("  var idcomments_post_url = '#{ options[:url] ? options[:url] : self.send( :"#{obj.class.to_s.underscore}_path", obj ) }';")
      concat_line raw('</script>')
      concat_line raw('<script type="text/javascript" src="http://www.intensedebate.com/js/genericLinkWrapperV2.js"></script>')
      nil
    end
